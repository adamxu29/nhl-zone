  ...
  return(
    <div className={`${tableCols} py-1 border-b border-gray-200 text-sm`}>
      <div className='flex items-center gap-3'>
        ...
        <img className='w-[45px] h-[45px] shrink-0 rounded-full' src={`https://assets.nhle.com/mugs/nhl/20262027/${goalie.teamAbbrevs.split(',').pop()!.trim()}/${goalie.playerId}.png`} alt={`${goalie.name} headshot`}/>
        <p className='font-semibold'>{goalie.name}</p>
        <span className='flex-1 text-xs text-gray-500'>{goalie.teamAbbrevs}</span>
      </div>
