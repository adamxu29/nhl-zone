...
function Game({ game }: { game: GameData}) {
  ...
  return(
    <div className='grid grid-cols-[150px_1fr_150px] my-5 h-35 w-300 border-1 border-dotted rounded-sm bg-white'>
      <div className='col-start-1 flex flex-col justify-center text-wrap ml-5'>
        <h1 className='font-semibold text-lg'>{formatGameDate(game.date, false)}</h1>
      ...
      <Link className='col-start-3 flex justify-end items-center mr-5' href={`https://www.nhl.com${game.game_center_link}`}>
        Gamecenter
      </Link>
    </div>
  )
...
async function TodaysGames() {
  ...
  return (
    <div className='bg-[#f2f2f2] min-h-screen'>
      <h1 className='font-bold text-4xl pl-10 py-10'>Todays Games</h1>

      <div className='flex flex-col items-center'>
        {games.length === 0 ? <strong className='text-2xl'>No Scheduled Games For Today</strong> : games.map((g) => {
